...
describe('BookPage', () => {
  ...
  it('блокирует отправку без контакта (клиентская валидация)', async () => {
    const user = userEvent.setup()
    mocks.useCreateBooking.mockReturnValue(baseBookingMock(() => {}))
    renderBookPage()
    ...
    expect(screen.getByText(/Укажите хотя бы один контакт/)).toBeInTheDocument()
    expect(mocks.useCreateBooking.mock.results[0].value.mutate).not.toHaveBeenCalled()
  })

  it('создаёт бронь при наличии контакта', async () => {
    const user = userEvent.setup()
